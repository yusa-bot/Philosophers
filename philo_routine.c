...
/*   Updated: 2026/02/21 17:10:31 by ayusa            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	long long	sleep_target_time;

	log_print(philo, "is sleeping");
	pthread_mutex_lock(&philo->meal_mutex);
	sleep_target_time = philo->last_eat_time
		+ philo->data->time_to_eat_us + philo->data->time_to_sleep_us;
	pthread_mutex_unlock(&philo->meal_mutex);
	ft_usleep_to_target_time(sleep_target_time);
}
